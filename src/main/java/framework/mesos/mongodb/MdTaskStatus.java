...
public class MdTaskStatus {
	private static final Logger LOGGER = Logger.getLogger(MdTaskStatus.class);
	...
	public MdTaskStatus(SerializableState state, FrameworkID frameworkID, TaskInfo taskInfo, StatePath statePath) {
	...
	public boolean taskInError() {
		...
		return MdTaskStatus.errorState(state);
	}
	...
	public void destroy() {
		try {
			state.delete(getKey());
			LOGGER.info("  destroy task  ");
		} catch (IOException e) {
			LOGGER.error("Could not destroy Task in ZK.");
		}
	}
}